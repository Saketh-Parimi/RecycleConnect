...
async def login(response: Response, form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    id = JSONEncoder().encode(user['_id'])
    id = id.replace('"', '')
    return {"status": "success", "user_id": id, "access_token": create_access_tokens(user['username']),
    ...
    if user_check == None:
        if len(user.username) > 3:
            if any(c in special_characters for c in user.password):
                ...
                user1 = await db.users.find_one({"username": user.username})
                user1['id'] = str(user1['_id']).replace('"', '')
                del user1['_id']
                
                response.status_code = status.HTTP_200_OK
                return {"status_code": status.HTTP_200_OK, "content": "success", "user_id": user1['id']}
            else:
